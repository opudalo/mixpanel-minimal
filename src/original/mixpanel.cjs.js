...
_.cookie = {
  ...
  set_seconds: function (
    ...
    document$1.cookie =
      name + '=' + encodeURIComponent(value) + expires + '; path=/' + cdomain + secure;
  },
...
Autocapture.prototype._initScrollDepthTracking = function () {
  ...
  var updateScrollDepth = function () {
    ...
    var scrollViewHeight =
      Math.max(document$1.documentElement.clientHeight, win.innerHeight || 0) + win.scrollY;
...
Autocapture.prototype.initPageviewTracking = function () {
  ...
  this.listenerLocationchange = safewrap(
    function () {
      ...
      var didPathChange =
        currentUrl.split('#')[0].split('?')[0] !== previousTrackedUrl.split('#')[0].split('?')[0];
  ...
  return this._mixpanel._track_or_batch(
    {
      ...
      endpoint:
        this._mixpanel.get_api_host('groups') + '/' + this._get_config('api_routes')['groups'],
  ...
  return this._mixpanel._track_or_batch(
    {
      ...
      data: date_encoded_data,
      endpoint:
        this._mixpanel.get_api_host('people') + '/' + this._get_config('api_routes')['engage'],
...
var ENQUEUE_REQUESTS =
  !USE_XHR && userAgent.indexOf('MSIE') === -1 && userAgent.indexOf('Mozilla') === -1;
...
MixpanelLib.prototype._init = function (token, config, name) {
  ...
  this.set_config(
    _.extend({}, DEFAULT_CONFIG, variable_features, config, {
      ...
      token: token,
      callback_fn:
        (name === PRIMARY_INSTANCE_NAME ? name : PRIMARY_INSTANCE_NAME + '.' + name) + '._jsc'
    })
  );
...
MixpanelLib.prototype._check_and_start_session_recording = addOptOutCheckMixpanelLib(function (
  force_start
) {
...
function loadThrowError(src, _onload) {
  throw new Error(
    'This build of Mixpanel only includes core SDK functionality, could not load ' + src
  );
}
