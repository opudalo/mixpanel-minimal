 ...
 * Utility functions
 */
const includes = (str: string, needle: string): boolean =>
...
class People implements MixpanelPeople {
  ...
  track_charge(
    amount: number,
    properties?: Properties | MixpanelCallback,
    callback?: MixpanelCallback
  ): void {
    const distinct_id = this.mixpanel.get_distinct_id()
    if (!distinct_id) {
...
function createClient(
  ...
  client._send_request = function (
    ...
    if (client.config.test) {
      console.log('[Minipanel Test Mode] Request intercepted:')
  ...
  client.identify = function (distinct_id: string): void {
    if (distinct_id.startsWith('$device:')) return

    const previous_distinct_id = client.persistence.props.distinct_id
    if (distinct_id && previous_distinct_id !== distinct_id) {
      client.persistence.register({ $user_id: distinct_id })
    }

    // Shlyapa. 🎩
    // if (!client.persistence.props.$device_id) {
    //   // The persisted distinct id might not actually be a device id at all
    //   // it might be a distinct id of the user from before
    //   const device_id = previous_distinct_id
    //   client.persistence.register_once(
    //     {
    ...
    if (previous_distinct_id !== distinct_id) {
      client.persistence.register({
        distinct_id
      })
    }
